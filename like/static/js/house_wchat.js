...
var FBrowser = (function () {
  ...
  return{isIE: !!window.attachEvent && !isOpera, isOpera: isOpera, isSafari: ua.indexOf('AppleWebKit/') > -1, isFirefox: ua.indexOf('Gecko') > -1 && ua.indexOf('KHTML') === -1, MobileSafari: /Apple.*Mobile.*Safari/.test(ua), isChrome: !!window.chrome};
})();
...
var FJsLoader = {load: function (sId, sUrl, fCallback, chset) {
  try {
...
var HouseWxBridgeReady = {init: function () {
  ...
  WeixinJSBridge.on("menu:share:appmessage", function (b) {
    WeixinJSBridge.invoke("sendAppMessage", {appid: "", img_url: g_info.coverPicUrl, img_width: "65", img_height: "65", link: document.location.href, desc: "【" + pagetitle + "】", title: g_info.houseName}, function (d) {
  ...
  WeixinJSBridge.on("menu:share:timeline", function (b) {
    WeixinJSBridge.invoke("shareTimeline", {img_url: g_info.coverPicUrl, img_width: "65", img_height: "65", link: document.location.href, desc: "【" + pagetitle + "】", title: g_info.houseName + '-' + pagetitle}, function (d) {
      WeixinJSBridge.log(d.err_msg);
    })
  });
}, shareWeibo: function () {
  ...
  WeixinJSBridge.on('menu:share:weibo', function (argv) {
    WeixinJSBridge.invoke('shareWeibo', {"content": g_info.houseName + "【" + pagetitle + "】" + document.location.href, "url": document.location.href}, function (res) {
...
var page_detail = {init: function () {
  ...
  function FoldPanel(id) {
    ...
    this.btn.onclick = function () {
      if (!_this.hidePanel.style.display || _this.hidePanel.style.display === "none") {
        ...
        this.innerHTML = '<em class="up"></em>';
      } else {
        _this.hidePanel.style.display = "none";
        this.innerHTML = '<em class="down"></em>';
      }
    };
  }
  ...
  var panel3 = new FoldPanel("c2");
  var panel4 = new FoldPanel("d6");
  var panel5 = new FoldPanel("c3");
}};
var page_kanfangsignup = {init: function () {
  function $(id) {
    return document.getElementById(id);
  }
...
var page_groupbuy = {init: function () {
  function $(id) {
    return document.getElementById(id);
  }
  ...
  if ($('bulidIntro') != null) {
    ...
    $('bulidIntro').onclick = function () {
      if (flag == false) {
        ...
        $('bulidIntro').innerHTML = '<em class="up"></em>';
      } else {
        flag = false;
        $('actInfoPart').style.display = 'block';
        $('actInfoAll').style.display = 'none';
        $('bulidIntro').innerHTML = '<em class="down"></em>';
      }
    };
  }
}};
var page_map = {map: null, housePosition: null, userPosition: null, positionTimerId: null, timeoutMillSec: 5000, isShowError: false, init: function () {
  ...
  var map, houselocation;
  var that = this;
  var container = document.getElementById("container");
  var position = new soso.maps.LatLng(lat, lng);
  this.housePosition = position;
  if (lat != '' && lng != '') {
    map = new soso.maps.Map(container, {center: new soso.maps.LatLng(lat, lng), zoomLevel: 15, mapTypeControl: false, panControl: false, zoomControl: false, });
    this.map = map;
    var label = new Label({map: map, position: position}, address);
    houselocation = new soso.maps.Marker({map: map, position: position});
  }
  else {
  }
  ...
  var directionsService = new soso.maps.DrivingService({complete: function (response) {
    var start = response.detail.start, end = response.detail.end;
    var anchor = new soso.maps.Point(6, 6), size = new soso.maps.Size(24, 36), user_icon = new soso.maps.MarkerImage('http://api.map.soso.com/v2/0/8c/theme/default/imgs/marker.png', size, new soso.maps.Point(0, 0), anchor);
    user_position_marker && user_position_marker.setMap(null);
    clearOverlay(route_lines);
    user_position_marker = new soso.maps.Marker({icon: user_icon, position: from, map: map, zIndex: 1});
    directions_routes = response.detail.routes;
    for (var i = 0; i < directions_routes.length; i++) {
      var route = directions_routes[i], legs = route;
      map.fitBounds(response.detail.bounds);
      var steps = legs.steps;
      route_steps = steps;
      polyline = new soso.maps.Polyline({path: route.path, strokeColor: '#3893F9', strokeWeight: 6, map: map})
...
var page_pic = {init: function () {
      ...
      if (typeof WeixinJSBridge == "undefined") {
        ...
        window.bigMapViewer = function () {
          ...
          for (var i = 0; i < thumbPicList.length; i++) {
            picInfoList.push({thumbPicUrl: thumbPicList[i], initPicUrl: 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==', style: '', imgHeight: 0, imgWidth: 0});
          }
          var bigPicIndex = 0;
          ...
          function relayoutIScroll(show) {
            ...
            var html = tmpl("iscroll_tmpl", {picList: picInfoList, liWidth: w, liHeight: h});
            $('#scroller').css('width', w * picInfoList.length + 'px');
            $('#scroller').html(html);
            if (!iscroll) {
              iscroll = new iScroll('gallery', {zoom: false, snap: true, momentum: false, hScrollbar: false, vScrollbar: false, fixScrollBar: false, hScroll: true, onScrollEnd: function () {
                if (this.dirX != 0) {
  ...
  function showBigPic(curPicUrl, picUrlList) {
    WeixinJSBridge.invoke("imagePreview", {current: curPicUrl, urls: picUrlList});
  }
...
var page_unitpic = {init: function () {
  ...
  function startMove() {
    ...
    var w = parseInt(oDiv.style.width), h = parseInt(oDiv.style.height);
    var L = oDiv.offsetLeft, T = oDiv.offsetTop;
    var w_DIF = W - w, h_DIF = H - h;
    if (w_DIF >= 0 && h_DIF >= 0) {
  ...
  function relayoutIScroll(show) {
    ...
    var html = tmpl("iscroll_tmpl", {picList: thumbPicList, liWidth: w, liHeight: h});
...
var page_feature = {init: function () {
  function adjustImg() {
...
if (document.addEventListener)
  document.addEventListener("click", registerZone2, false); else if (document.attachEvent)
